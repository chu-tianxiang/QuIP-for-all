...
def round_allbal(
    ...
    for ip in range(npasses):
        ...
        if ((w_hat == wr).all()):
            sys.stderr.write(
                f"breaking after {ip+1} greedy passes found fixed point")
            break
        w_hat.copy_(wr)
...
def round_ldl(w, H, nbits, n_greedy_passes=9, unbiased=False):
    ...
    assert (not unbiased) or (
        n_greedy_passes
        == 0), "greedy passes are incompatible with unbiased LDL rounding"
    (d, d_) = H.shape
    assert (d == d_)
    (m, d) = w.shape
    L = torch.linalg.cholesky(H).to(w.dtype)
    ...
    for i in reversed(range(d)):
        w_hat[:, i] = torch.clamp(
            torch.floor(w[:, i] + (w[:, i:] - w_hat[:, i:]) @ L[i:, i] +
                        eta[:, i]),
            min=0,
            max=2**nbits - 1)
    ...
    for igp in range(n_greedy_passes):
        ...
        if ((w_hat == wr).all()):
            sys.stderr.write(
                f"breaking after {igp+1} greedy passes found fixed point")
            break
        w_hat.copy_(wr)
...
def round_sorted_ldlqRG_block(w,
                              H,
                              nbits,
                              n_greedy_passes=9,
                              unbiased=False,
...
def round_ldl_block(w,
    ...
    for i2 in range(d, 0, -blocksize):
        ...
        for i in reversed(range(count)):
            WHat1[:, i] = torch.clamp(
                torch.floor(W1[:, i] + (W1 - WHat1) @ L1[i1:i2, i] +
                            W2Hdiff @ L1[i2:, i] + Eta1[:, i]),
                min=0,
                max=2**nbits - 1)
...
def ldlp_admm(H, rho=0.1, niters=100):
    ...
    for ii in range(100):
        ...
        C = torch.diag(1 / torch.max(torch.tensor(1.0, device=H.device),
                                     ((X + W).T @ (X + W)).diag().sqrt()))
        Z = (X + W) @ C
        W = W + X - Z
        objective = ((Z + torch.eye(n, device=H.device)) @ H
                     @ (Z + torch.eye(n, device=H.device)).T).trace()
...
def round_ldl_admm(w, H, nbits, n_greedy_passes=9, unbiased=False):
    ...
    for i in range(d):
        w_hat_next = torch.clamp(torch.floor(w_hat - (w_hat - w) @ L + eta),
                                 min=0,
                                 max=2**nbits - 1)
    ...
    for jj in range(n_greedy_passes):
        ...
        for ii in range(d):
            wr_prev = wr.clone()
            wr = torch.clamp(torch.round(wr_target + (w - wr) @ HnM),
                             min=0,
                             max=2**nbits - 1)
            if (wr == wr_prev).all():
