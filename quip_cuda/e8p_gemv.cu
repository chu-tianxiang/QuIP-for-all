...
decode_matvec_e8p_kernel(
    ...
    for (int iin = blockIdx.x; iin < (N >> 4); iin += gridDim.x) {
        ...
        if ((laneId & 1) == 0) {
            atomicAdd(output + (iin << 4) + (laneId >> 1), (laneId & 2) ? __float2half_rn(z2) : __float2half_rn(z0));
        }
...
__host__ extern torch::Tensor decode_matvec_e8p(
    ...
    int grid_size;
    cudaDeviceGetAttribute(&grid_size, cudaDevAttrMultiProcessorCount, x.get_device());
...
__host__ extern torch::Tensor decompress_packed_e8p(
    ...
    int grid_size;
    cudaDeviceGetAttribute(&grid_size, cudaDevAttrMultiProcessorCount, weights_compressed.get_device());
    at::cuda::CUDAStream stream = at::cuda::getCurrentCUDAStream();
